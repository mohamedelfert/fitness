...
| FR-BIO-*, FR-AN-001/005 | Progress + AI analysis + biometrics + photos + weekly report | 🟡 **Biometrics ✅** (`FR-BIO-001`, TDD, 7 tests): new **Biometrics** module — `biometrics` table (append-only, idempotent on `client_ulid`, Plane A); `POST /v1/biometrics` + `GET /v1/biometrics?type=` (newest-first, person-scoped); `Biometric::TYPES` = weight/body_fat/circumference-sites. **Progress analysis ✅** (`FR-AN-001`, TDD, 6 tests): new **Analytics** module — `GET /v1/me/progress` returns per-metric trend (first/latest/change/least-squares `trend_per_week`/samples over a 90d window) + per-active-goal projection. Goal→series is **unit-driven** (kg/lb→weight, %→body_fat); reps/time/distance → `no_matching_metric` (those are performance trends FR-AN-003, separate). Projection statuses: `projected` (ETA + `on_track` vs `target_date`) / `trend_unfavorable` (flat or moving away → no date) / `insufficient_data` / `no_matching_metric`. **Computed on read** (cheap person-scoped point queries, like the nutrition summary — NOT the population-scale scoring of DATABASE_DESIGN §3.5; `ponytail:` materialise+queue only if a history grows large enough to hurt). **Adherence analytics ✅** (`FR-AN-002`, TDD, 6 tests): `GET /v1/me/adherence` — training + nutrition-logging activity over a 28d window (`sessions`, `active_days`, `sessions_per_week`, `nutrition_logged_days`, `current_streak_days`) plus `adherence_pct` vs the active program's planned weekly cadence (= its workout count); **no active program → planned/pct null** (no fabricated denominator). Streak counts consecutive session-days back from today with **one day of grace** (a not-yet-trained today doesn't break a streak that ran through yesterday). Same on-read rationale as progress (not the §3.5 `adherence_events` churn read-model, which stays P2). **Weekly AI report ✅** (`FR-AN-005`, TDD, 9 tests): `GET /v1/me/reports/weekly` — advisory weekly narrative grounded in the progress + adherence read-models (`WeeklyReportGenerator` in AiOrchestration depends on both Analytics services). Advisory-only like daily-rec/recovery (no contraindication sandwich; safety by construction via the prompt). **Materialised once per ISO week** (`weekly_reports`, unique `person_id+iso_week` via Carbon `o-\WW`, year-boundary safe): the week's first call generates + debits one credit; same-week refreshes are cache-served, free, no second model call (NFR-AI-001). Gate (403) + onboarding (422) + 402-on-miss + blank-output 422 (never 500, not charged); `default` tier (flagship retention surface). **E1.7 (AI analytics) complete.** **Progress photos (signed-URL/encrypted) still ⬜** (needs object storage). |
| FR-BIO-003, FR-AI-005 | Wearables ingest + recovery tips | 🟡 **Wearable ingest ✅** (`FR-BIO-003`, TDD, 7 tests): new **Wearables** module — `wearable_streams` (append-only, per-reading idempotent on `client_ulid`, Plane A); **batch** `POST /v1/wearables/ingest` (one SELECT + one bulk INSERT, dedup within-batch + cross-request, returns `{ingested, skipped}`) + `GET /v1/wearables?metric=` (newest-first, capped). `METRICS` = hr/hrv/sleep/steps. Apple Health/Health Connect connectors are client-side; `connect` OAuth handshake deferred. **Recovery tips ✅** (`FR-AI-005`, `GET /v1/ai/recovery`, 9 tests) — advisory recovery readout grounded in the latest wearable signals (per-metric, last 7d) + optional `?soreness=`; advisory-only like daily-rec (no sandwich, safety by construction), generated fresh each call (reflects live data; cost bounded by metering not caching), `cheap` tier, metered (`feature=recovery`). |
| FR-ENG-001/002/003 | Goals, habits, streaks/XP | 🟡 **Goals ✅** (`FR-ENG-001`, complete): `GET/POST /v1/goals` (from onboarding) + **projected attainment** via `GET /v1/me/progress` (E1.7) + **manual close** `PATCH /v1/goals/{id}` (status active/achieved/abandoned or edit targets; person-owned → cross-person 404; auto-achievement deliberately deferred — needs a trigger on every biometric write). **Habit tracking ✅** (`FR-ENG-002`, TDD, 7 tests): two tables — `habits` (parent: name, cadence daily/weekly, target_per_period, active) + append-only idempotent `habit_logs` (client_ulid, ADR-005). `GET/POST /v1/habits` (each habit carries `current_streak` — consecutive days with a completion, one day of grace; the AdherenceAnalyzer streak math inlined, 2nd use) + `POST /v1/habit-logs` (person-owned habit → unknown/cross-person 404). **Behavioural nudge ✅** (`GET /v1/ai/habit-nudge`, TDD, 7 tests) — advisory contextual nudge grounded in active habits + each habit's streak/logged-today; mirrors recovery/daily-rec (advisory, no sandwich, safety by construction), fresh each call, `cheap` tier, metered (`feature=habit_nudge`). FR-ENG-002 complete. **XP/levels/streaks/badges ✅** (`FR-ENG-003`, TDD, 8 tests): `GET /v1/me/gamification` — XP/level/`streak_days` computed **on read** from append-only activity (sessions + habit logs only). On-read is correct *because* the source logs are append-only (INV-002) → total XP is deterministic + monotonic. Flat level curve (`config/gamification.php`: `xp_per_level`, per-source `points`). Streak reuses the extracted **`App\Support\DayStreak`** helper (rule-of-three: AdherenceAnalyzer + HabitController rewired onto it). `ponytail:` flip to the DATABASE_DESIGN `xp_ledger` + per-module listeners when XP becomes spendable / notifications need award-timestamps / sources outgrow ~3. **Badges ✅** — a *historical* award, so **persisted in `person_badges`** (NOT recomputed): catalog is config (`gamification.badges`, slug→name→stat≥gte), awarded + persisted on first threshold-cross (awarded on read at the gamification GET, idempotent via unique `(person_id, badge_slug)`), returned in the `badges[]` field; `ponytail:` event-driven awarding only if exact earn-time matters. |
