...
### E1.8 — Engagement  🟡
- Goals tracking (`FR-ENG-001`) ✅ — create/list/projected-attainment (E1.7) + manual close (`PATCH /v1/goals/{id}`). Habits (`FR-ENG-002`) ✅ — `habits` + append-only `habit_logs`, `GET/POST /v1/habits` (with `current_streak`) + `POST /v1/habit-logs`; **behavioural nudge** `GET /v1/ai/habit-nudge` (advisory-AI, grounded in habits+streaks, metered). XP/levels/streaks/badges (`FR-ENG-003`) ✅ — `GET /v1/me/gamification`, XP/level/streak derived on read from append-only activity (sessions + habit logs); **badges persisted** in `person_badges` (catalog = config, awarded on first threshold-cross, never recomputed); streak math extracted to `App\Support\DayStreak`. Smart notifications (`FR-ENG-006`) ⬜ — **blocked on push infra** (device tokens + FCM/APNs), same class as progress-photos / wearables-connect.
