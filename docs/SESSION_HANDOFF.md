...
> **Last updated:** 2026-06-19 (AICredit wallet + meter debit on `POST /v1/ai/program`) · **Repo:** local `/home/mohamed/Desktop/work/fitness` → **remote** `github.com/mohamedelfert/fitness` (branch `main`).
...
**E1.6 — AICredit wallet + meter shipped (on `main`, TDD, suite now 93 tests / 285 assertions, 3 Filament skips):** `ai_credit_wallets` + `ai_credit_ledger` (DATABASE_DESIGN §2.5) + **`AiCreditMeter`** — single-entry *signed* ledger (NOT the Plane-B double-entry money ledger; INV-003 does not apply), atomic `lockForUpdate` read-modify-write debit (never goes negative), **debit-once-on-success after persist** (the safety reject+regenerate loop and any 422 are free to the user — verified by test). `POST /v1/ai/program` now: gate (403) → onboarding (422) → **`ensureCanAfford` (402)** → generate → debit (ref = program). `GET /v1/me/ai-credits` auto-provisions an empty wallet. `InsufficientCreditsException` self-renders 402. Config `ai.credits` (`default`/`program`/`free_grant`). OpenAPI updated (402 on `/ai/program`, new `/me/ai-credits` + `AiCreditWallet` schema; contract guard green).
**Two decisions captured (in PHASE_PLAN E1.6) — settle before the next slice:**
- **AICredit funding trigger** — wallets start *empty*; nothing grants `free_grant` in production yet (tests fund explicitly via the meter; the 402 path is the natural unfunded case). Pre-billing stopgap: pick grant-on-onboarding-completion (cleanest via an event so Identity doesn't depend on AiOrchestration) vs defer to E1.9 plan grants.
- **MealPlan gen food-grounding** — library-referenced (add `slug` + dietary tags to `food_items` → real dietary safety scan) vs free-form meals with macros (`meal_plan_items.food_item_id` already nullable; simpler but weaker post-eval).

**Phase 1 — recommended next (per PHASE_PLAN intra-phase order):** **E1.6 cont'd**: MealPlan generation (same safety sandwich, `meal_plans` staged + the meter already wraps it — settle the food-grounding decision first), model-tiering (cheap-for-swaps), RAG A/B, streaming. Then **E1.5 biometrics/progress photos/wearables** (unblocked). When Q5 lands: bind a real `LlmGateway` adapter in `AiOrchestrationServiceProvider`, add `config/ai.php` pricing, run the spike's quality/cost/latency measurement (`docs/AI_BRAIN_SPIKE.md`).
