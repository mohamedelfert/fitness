...
> **Last updated:** 2026-06-19 (AICredit wallet + meter; AI meal-plan + exercise-alternatives generation) · **Repo:** local `/home/mohamed/Desktop/work/fitness` → **remote** `github.com/mohamedelfert/fitness` (branch `main`).
...
**Phase 1 — recommended next (per PHASE_PLAN intra-phase order):** **E1.6 refactor now due** — three concrete generators exist (`ProgramGenerator`, `MealPlanGenerator`, `ExerciseAlternativeGenerator`); extract the shared loop (generate→parse→resolve→scan→reject/regenerate) + `ai_interactions` logging/cost into a base, plus a `SafetyScanner` interface (`unsafeSlugs(Person, iterable): array`, already shared by Contra/Dietary scanners). **Note the shapes differ**: program/meal-plan persist a graph + return a model + wrap a transaction; exercise-alternatives takes extra inputs, persists nothing, returns proposals (200) — so the base must not bake in "persist a graph" (let `finalize()` vary). Do it as a pure behavior-preserving refactor under the green suite. Then more E1.6 (coach-chat/plan-adjust/daily-rec, RAG A/B, streaming) + the AICredit funding-trigger wiring above. Then **E1.5 biometrics/progress photos/wearables** (unblocked). When Q5 lands: bind a real `LlmGateway` adapter in `AiOrchestrationServiceProvider`, add `config/ai.php` pricing, run the spike (`docs/AI_BRAIN_SPIKE.md`).
