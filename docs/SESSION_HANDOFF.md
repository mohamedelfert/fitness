...
**Phase 1 — recommended next (per PHASE_PLAN intra-phase order):** **E1.6 cont'd**: MealPlan generation (same sandwich, `meal_plans` staged), **AICredit wallet + meter debit** per generation, model-tiering (cheap-for-swaps), RAG A/B, streaming. Then **E1.5 biometrics/progress photos/wearables** (unblocked). When Q5 lands: bind a real `LlmGateway` adapter in `AiOrchestrationServiceProvider`, add `config/ai.php` pricing, run the spike's quality/cost/latency measurement (`docs/AI_BRAIN_SPIKE.md`).
